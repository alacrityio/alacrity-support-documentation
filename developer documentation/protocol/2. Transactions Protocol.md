...
> **'delayed' status**   The delayed status only applies to delayed user transactions, that is, explicit user-created transactions that have a delay to satisfy authorizations (see 3.6.3. Delayed User Transactions for more information).
...
> **Deprecation Notice**   Deferred transactions are deprecated as of ALAIO 2.0. For more details on their behavior, refer to the [Deferred Transactions]() section on the `alaio.cdt` documentation.
